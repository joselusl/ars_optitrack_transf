...
class ArenaObstaclesROS:
    ...
    obstacles_info_file_path = pck_path + "/config/arena_obstacles_dynamic.yaml"
    ...
    def __init__(self):
        rospy.init_node("Optitrack_obstacles_node_dynamic")
    ...
    def timer_pub_callback(self, time_stamp):
        ...
        if self.obstacles_sizes:
            for i, sizes in enumerate(self.obstacles_sizes):
                obst_pos = self.read_tf_data("world", f"dynamic{i+1}", rospy.Time())
                if obst_pos is None:
                    continue
